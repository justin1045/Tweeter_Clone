...
const ProfilePage = () => {
  ...
  const isAlreadyFollowing = authUser?.following?.includes(user?._id);
  ...
  const { mutate: updateProfile, isPending: isUpdatingProfile } = useMutation({
    ...
    onSuccess: async () => {
      ...
      await refetch(); // ✅ update UI with latest profile
  ...
  const handleImgChange = (e, type) => {
    ...
    reader.onload = () => {
      if (type === "cover") setCoverImagePreview(reader.result);
      else setProfileImagePreview(reader.result);
    };
  ...
  return (
    <div className="flex-[4_4_0] border-r border-gray-700 min-h-screen">
      ...
      <div className="relative group/cover">
        <img
          src={coverImagePreview || user.coverImage || "/cover.png"}
        ...
        <div className="avatar absolute -bottom-16 left-4">
          <div className="w-32 rounded-full relative group/avatar">
            <img
              src={profileImagePreview || user.profileImage || "/avatar-placeholder.png"}
      ...
      <div className="flex justify-end px-4 mt-5">
        ...
        {!isMyProfile && (
          <button
            ...
            disabled={isPending}
          >
            {isPending ? "Processing..." : isAlreadyFollowing ? "Unfollow" : "Follow"}
          </button>
        )}
